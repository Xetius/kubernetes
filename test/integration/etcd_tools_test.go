...
func TestSet(t *testing.T) {
	...
	etcdStorage := etcdstorage.NewEtcdStorage(client, testapi.Default.Codec(), "", false, genericapiserver.DefaultDeserializationCacheSize)
	ctx := context.TODO()
...
func TestWatch(t *testing.T) {
	client := framework.NewEtcdClient()
	keysAPI := etcd.NewKeysAPI(client)
	etcdStorage := etcdstorage.NewEtcdStorage(client, testapi.Default.Codec(), etcdtest.PathPrefix(), false, genericapiserver.DefaultDeserializationCacheSize)
	ctx := context.TODO()
	framework.WithEtcdKey(func(key string) {
