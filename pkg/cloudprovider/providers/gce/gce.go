...
func (gce *GCECloud) EnsureLoadBalancer(service *api.Service, hostNames []string) (*api.LoadBalancerStatus, error) {
	...
	glog.V(2).Infof("EnsureLoadBalancer(%v, %v, %v, %v, %v, %v)", name, gce.region, loadBalancerIP, portStr, hosts, serviceName)
	...
	firewallExists, firewallNeedsUpdate, err := gce.firewallNeedsUpdate(name, gce.region, ipAddress, ports)
	if err != nil {
		return nil, err
	}
	...
	if firewallNeedsUpdate {
		...
		if firewallExists {
			if err := gce.updateFirewall(name, gce.region, desc, "0.0.0.0/0", ports, hosts); err != nil {
				return nil, err
			}
			glog.V(4).Infof("EnsureLoadBalancer(%v(%v)): updated firewall", name, serviceName)
		} else {
			if err := gce.createFirewall(name, gce.region, desc, "0.0.0.0/0", ports, hosts); err != nil {
				return nil, err
			}
...
func (gce *GCECloud) EnsureLoadBalancerDeleted(service *api.Service) error {
	glog.V(2).Infof("EnsureLoadBalancerDeleted(%v)", *service)
